...
RESULT_DIR="$OUTPUT_PATH/$apk_file_name.kea.result.$DEVICE_SERIAL.$AVD_NAME#$current_date_time"
...
# 从第8个参数开始，将其余的作为 APK 文件列表
# APK_FILES=("${@:8}")
# # 安装应用并自动授予权限
# # 使用 adb install-multiple 命令安装 APK 文件
# adb -s $DEVICE_SERIAL install-multiple -g "${APK_FILES[@]}"&> $RESULT_DIR/install.log
adb -s $DEVICE_SERIAL install $APK_PATH &> $RESULT_DIR/install.log

sleep 2
# #anroid 11 通过以下命令授予存储权限（在Android10报错）
...
# start logcat
echo "** START LOGCAT (${DEVICE_SERIAL}) "
adb -s $DEVICE_SERIAL logcat -c
adb -s $DEVICE_SERIAL logcat AndroidRuntime:E CrashAnrDetector:D System.err:W CustomActivityOnCrash:E ACRA:E WordPress-EDITOR:E *:F *:S > $result_dir/logcat.log &
