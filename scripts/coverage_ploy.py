...
def compute_average_code_coverage(target_app_testing_result_dirs):
    original_coverage_ec_files = get_coverage_ec_files_str(target_app_testing_result_dirs)
    ...
    average_coverage_data_summary_file_path = os.path.join(target_app_testing_result_dirs,"all_coverage.csv")
    ...
    for coverage_ec_file in average_coverage_dict:
        ...
        csv_file.close()
    return average_coverage_data_summary_file_path
...
def plot_coverage(coverage_dir):
    """绘制覆盖率随时间变化的折线图"""
    coverage_csv = os.path.join(coverage_directory,"all_coverage.csv")
    ...
    # 提取覆盖率数据，从第四列开始
    coverage_data = df_sorted.iloc[:, 3:7].reset_index(drop=True)  # 选择第4到第7列

    # 设置列名
    coverage_data.columns = ['Line Coverage', 'Branch Coverage', 'Method Coverage', 'Class Coverage']

    # 绘制折线图
    plt.figure(figsize=(12, 6))

    # 绘制每种覆盖率
    for column in coverage_data.columns:
        plt.plot(coverage_data.index + 1, coverage_data[column], marker='o', label=column)
    app_name = get_app_name(coverage_dir)
    tool_name = get_tool_name(coverage_dir)
    timestamp = get_timestamp(coverage_dir)
    plt.title('{}_{}_{}'.format(app_name,tool_name,timestamp))
    ...
    plt.tight_layout()
    # 保存折线图
    plt.savefig('D:/code_coverage_over_time/{}.png'.format(coverage_directory.split('\\')[-1]), dpi=300)  # 保存为 PNG 格式，分辨率为 300 DPI
...
if __name__ == "__main__":
    coverage_directory = r"D:\GitHubRepo\droidbot-gpt\output\MaterialFBook\#224_time_60\MaterialFBook4.0.2-debug-#224.apk.hybirddroid.result.emulator-5558.test_hybirddroid_android7#2024-07-26-18-42-25" # 替换为你的 .ec 文件目录
    coverage_data = compute_average_code_coverage(coverage_directory)
    if coverage_data:
        plot_coverage(coverage_directory)
    else:
