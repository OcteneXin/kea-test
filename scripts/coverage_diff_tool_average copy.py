...
ALL_APPS = ['ActivityDiary', 'AmazeFileManager', 'and-bible', 'AnkiDroid', 'APhotoManager', 'commons',
            ...
            'sunflower', 'WordPress',"AntennaPod","newpipe","duckduckgo",'redreader','myExpenses', 'bookdash', 'Wikipedia', 'kotatsu', 'aard2','vanilla','feeder']
TOOL_NAMES = ['hybirddroid','fastbot','monkey','Aurora','ape','kea']
...
def get_combined_all_and_average_coverage_csv(root_dir):
    ...
    print("all_columns length:", len(all_columns))  # 检查列表长度
    print("all_columns content:", all_columns)  # 打印内容
    # 横向合并所有的第一列
    combined_df = pd.concat(all_columns, axis=1)
    # 用最后一个有效值填充缺失值
    combined_df.fillna(method='ffill', inplace=True)
...
def get_combined_only_average_coverage_csv(root_dir):
    all_average_coverage_csvs = [os.path.join(root_dir, f) for f in os.listdir(root_dir) if
                         os.path.isfile(os.paurath.join(root_dir, f)) and f.endswith('.csv') and f.startswith('all_average')]
       # 指定要读取的目录
    # root_dir = 'path/to/your/directory'
